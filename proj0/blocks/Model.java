...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        if (piece.height()>(_height - row) || piece.width()> (_width-col)){
            return false;
        }
        for (int r = row; r < height(); r++) {
            for (int c = col; c < width(); c++) {
                if (piece.get(r, c) && _cells[r][c]) {
                    return false;
                }
            }
        }
    ...
    void place(Piece piece, int row, int col) {
        assert placeable(piece, row, col);package blocks;
 ...
 */
        class Model {
            ...
            Model(blocks.Model model) {
                ...
                _current = model._current;
                _lastHistory = model._lastHistory;
                for (blocks.Model.GameState g : model._history.subList(0, model._lastHistory + 1)) {
                    _history.add(new blocks.Model.GameState(g));
                }
            }
            ...
            int[][] rowColumnCounts() {
                ...
                for (int r=0;r<_height;r++){
                    for (int c=0; c < _width;c++){
                        if (!get(r,c)){
                            int [] rowarray = result[0];
                            int [] colarray = result [1];
                            r= row;
                            c = col;
                            rowarray[r] = rowarray [r] +1;
                            colarray[c] = colarray [c] +1;
                        }
                    }
                }
                return result;
            ...
            private int scoreClearedLines(int nrows, int ncols) {
                return 0; //*piece boxes + (clearedcolumn * _height)  + (clearedrow * _width) + (clearedcolumn+clearedrow) * clearstreak */
            }
            ...
            void pushState() {
                ...
                if (_current >= _history.size()) {
                    _history.add(new blocks.Model.GameState());
                }
            ...
            private class GameState {
                ...
                /** A copy of STATE. */
                GameState(blocks.Model.GameState state) {
                    _savedCells = new boolean[_height][_width];
            ...
            private ArrayList<blocks.Model.GameState> _history = new ArrayList<>();
